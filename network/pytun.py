...
__credits__ = ["Gawen Arab", "Ben Lapid", "Dmitriy Kuptsov"]
__license__ = "MIT"
__version__ = "1.0.1"
...
class Tunnel(object):
	...
	def __init__(self, mode = None, pattern = None, auto_open = True, no_pi = True):
	...
	def open(self):
		...
		try:
			ret = fcntl.ioctl(self.fd, self.TUNSETIFF, struct.pack("16sH", bytes(self.pattern, encoding="ascii"), self.mode | self.no_pi))
		except IOError as e:
			if e.errno == 1:
			...
			raise
		self.name = ret[:16].strip(bytearray([0x0]))
	...
	def set_ipv6(self, ip):
		...
		# Work around for the IPv6 addresses
		system("ifconfig %s add %s" % (self.name.decode("ascii"), ip));
		ifreq = struct.pack('16sH', self.name, self.IFF_UP|self.IFF_POINTOPOINT|self.IFF_RUNNING|self.IFF_MULTICAST)
		fcntl.ioctl(sock, self.SIOCSIFFLAGS, ifreq)

	def set_mtu(self, mtu):
		system("ifconfig %s mtu %s" % (self.name.decode("ascii"), mtu))    
